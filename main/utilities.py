...
def transform_csv(csv_contents: pd.DataFrame) -> pd.DataFrame:
    csv_contents["LivingSpaceGreaterThan2000"] = np.where(
        csv_contents[' "Living Space (sq ft)"'] > 2000, True, False
    )
    ...
    csv_contents = csv_contents.drop(
        csv_contents[csv_contents["ListPriceInclTax"] < 130000].index
    )
